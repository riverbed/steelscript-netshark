...
files and directory in NetShark
"""
...
from steelscript.netshark.core._interfaces import loaded, _InputSource
import datetime
...
class _FSResource(object):
    ...
    def __init__(self, shark, data):
        ...
        # Here it saves the netshark reference so the caller does not have to
        ...
        if len(data) < 2 or 'created' not in data:
            ...
            # uploading a trace file). Only NetShark can control the root
            # directory
...
class Directory4(_FSResource):
    ...
    def create(cls, shark, path):
        ...
        raise NetSharkException(error_msg + path + \
...
class FileMeta4(type):
    ...
    files in a netshark appliance with the correct subclass
    """
...
class _AggregatedFile(File4):
    ...
    def _create_aggregated_file(cls, type, shark, path, files):
        ...
        if type != "MULTISEGMENT_FILE" and type != "MERGED_FILE":
            raise NetSharkException(error_msg + \
