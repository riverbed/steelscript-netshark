...
class NetShark(Service):
    ...
    def __init__(self, host, port=None, auth=None,
        ...
        if force_version is not None:
            if not force_version in API_TABLE:
                raise NetSharkException("API version %s unsupported" % force_version)
        ...
        super(NetShark, self).__init__("shark", host, port=port, auth=auth,
                                       versions=versions)
    ...
    def __repr__(self):
        if self.port is not None:
            return '<NetShark %s:%d>' % (self.host, self.port)
        else:
            return '<NetShark %s>' % (self.host)
    ...
    def get_interfaces(self, force_refetch=False):
        ...
        capture interfaces available on the NetShark.

        If ``force_refetch`` is True, the list of interfaces will be
        re-fetched from the NetShark.  Otherwise, the list may be cached.
        """
    ...
    def get_capture_jobs(self, force_refetch=False):
        ...
        capture jobs currently running on the NetShark.
        ...
        if self._capture_jobs_cache is None or force_refetch:
            ...
            for j in self._capture_jobs_cache:
                if j.name == jobname:
                    ...
                    break
        if job is None:
            raise ValueError('No Job called {0} is available on this NetShark'.format(jobname))
        else:
            return job

    def get_clips(self, force_refetch=False):
        """Return a list of TraceClip objects, corresponding to the
        trace clips available on the NetShark.
    ...
    def get_trace_clip_by_description(self, description, force_refetch=False):
        ...
        **Note**: Clips don't have descriptions by default. A description can
        ...
        if len(clips) == 0:
            raise NetSharkException('cannot find clip %s' % description)

        return clips[0]

    def get_files(self, force_refetch=False):
        """Return a list of TraceFile, MergedFile or Multisegment objects,
        corresponding to the trace files available on the NetShark.
    ...
    def get_logininfo(self):
        ...
        logging in to the netshark, including the protocol version,
